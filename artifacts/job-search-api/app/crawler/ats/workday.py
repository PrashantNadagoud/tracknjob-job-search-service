...
_PAGE_SIZE = 20
_DEFAULT_WD_API = "https://{slug}.wd1.myworkdayjobs.com/wday/cxs/{slug}/External/jobs"
_DEFAULT_WD_HOST = "https://{slug}.wd1.myworkdayjobs.com"


def _resolve_workday_urls(ats_slug: str) -> tuple[str, str]:
    """Return (api_url, host_base) for a Workday slug or full URL override.

    When the dispatcher passes ``crawl_url`` as ``ats_slug`` (starts with
    ``http``), it is used directly as the POST API endpoint. The link base
    is always just ``scheme://netloc`` so that ``external_path`` (which starts
    with ``/``) is appended without duplicating any path segments.

    Returns:
        api_url:   POST endpoint for the Workday CXS jobs API.
        host_base: ``scheme://netloc`` used as prefix for job page URLs.
    """
    ...
    if ats_slug.startswith("http"):
        ...
        return ats_slug, host_base
    else:
        api_url = _DEFAULT_WD_API.format(slug=ats_slug)
        host_base = _DEFAULT_WD_HOST.format(slug=ats_slug)
        return api_url, host_base
...
class WorkdayCrawler(BaseATSCrawler):
    ...
    async def crawl(
        ...
        offset = 0
        api_url, host_base = _resolve_workday_urls(ats_slug)
        # Use the first hostname segment for display (e.g. "amazon" from amazon.wd3...)
        from urllib.parse import urlparse
        display_slug = urlparse(ats_slug).netloc.split(".")[0] if ats_slug.startswith("http") else ats_slug
        ...
        while True:
            ...
            for p in postings:
                ...
                source_url = (
                    f"{host_base}{external_path}" if external_path else api_url
                )
